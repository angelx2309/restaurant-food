...
(function ($) {
  ...
  var ProductManager = (function () {
    ...
    */
    ...
    var addProduct = function (id, name, summary, price, quantity, image, variante) {
      var products = getAllProducts();
      products.push({
        id: id,
        name: name,
    ...
    */
    ...
    var setProduct = function (id, name, summary, price, quantity, image, variante) {
      ...
      if (!updatePoduct(id, quantity, true)) {
        addProduct(id, name, summary, price, quantity, image, variante);
      }
    };
  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    var idGrandTotal2 = 'my-cart-grand-total2';
    var idEmptyCartMessage = 'my-cart-empty-message';
    var idDiscountPrice = 'my-cart-discount-price';
    var idDiscountPrice2 = 'my-cart-discount-price2';
    ...
    if (options.cartItems && options.cartItems.constructor === Array) {
      ...
      $.each(options.cartItems, function () {
        ProductManager.setProduct(this.id, this.name, this.summary, this.price, this.quantity, this.image, this.variante);
      });
    }
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h4 class="modal-title" id="myModalLabel"><span class="fas fa-shopping-cart"></span> Mi Pedido</h4>' +
        '</div>' +
        ...
        '<div id="factura">'+
          ...
          '<table class="table table-hover table-responsive">'+
            '<tr> <th>Cliente</th> <th>Contacto</th> <th>Dirección</th> <th>Fecha</th> </tr>'+
            '<tr> <td>Cliente</td> <td>Contacto</td> <td>Dirección</td> <td>Fecha</td> </tr>'+
          '</table>'+
          '<table class="table table-hover table-responsive" id="' + idCartTable2 + '"></table>' +
        ...
        '<button type="button" class="btn btn-primary nextCart">Siguiente</button>' +
    ...
    var drawTable = function () {
      ...
      $cartTable2.append(
        '<tr><th></th>  <th>Producto</th>  <th>Precio</th>  <th>Cantidad</th>  <th>Total</th> </tr>'
      );
      ...
      $.each(products, function () {
        ...
        $cartTable2.append(
          ...
          '<td>' + this.name + ' ('+this.variante+')  </td>' +
          '<td title="Unit Price" class="text-right">' + options.currencySymbol + MathHelper.getRoundedNumber(this.price) + '</td>' +
          '<td title="Quantity"><input type="number" disabled min="1" style="width: 70px;" class="' + classProductQuantity + '" value="' + this.quantity + '"/></td>' +
          '<td title="Total" class="text-right ' + classProductTotal + '">' + options.currencySymbol + MathHelper.getRoundedNumber(total) + '</td>' +
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '</tr>' :
        '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">Carrito vacio</div>'
      );
      $cartTable2.append(
        '<tr>' +
        '<td></td>' +
      ...
      if (products.length && discountPrice !== null) {
        ...
        $cartTable2.append(
          ...
          '<td></td>' +
          '<td></td>' +
          '<td class="text-right"><strong id="' + idDiscountPrice2 + '"></strong></td>' +
          '<td></td>' +
          '</tr>'
        );
      }
    ...
    var showGrandTotal = function () {
      $("#" + idGrandTotal+","+"#" + idGrandTotal2).text(options.currencySymbol + MathHelper.getRoundedNumber(ProductManager.getTotalPrice()));
    };
    var showDiscountPrice = function () {
      $("#" + idDiscountPrice +","+ "#" + idDiscountPrice2).text(options.currencySymbol + MathHelper.getRoundedNumber(options.getDiscountPrice(ProductManager.getAllProducts(), ProductManager.getTotalPrice(), ProductManager.getTotalQuantity())));
    };
    ...
    */
    ...
    $(document).on('click', targetSelector, function () {
      ...
      ProductManager.setProduct(id, name, summary, price, quantity, image, variante);
      $cartBadge.text(ProductManager.getTotalQuantity());

      options.afterAddOnCart(ProductManager.getAllProducts(), ProductManager.getTotalPrice(), ProductManager.getTotalQuantity());
    });

    $(document).on('click', '.nextCart', function() {
      $('#my-cart-table').hide();
      $('#clientInfo').show();
    });
